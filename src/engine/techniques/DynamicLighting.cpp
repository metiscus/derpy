...
// parameters
static const char *lightingVertexShaderText = "vec3 computeLighting(mat4 viewMatrix, vec4 vertex) {\n"
                                              "  return (viewMatrix * vec4(vertex.xyz, 1.f)).xyz;\n"
                                              "}\n";

// parameters
//	1 - number of lights that are active
static const char *lightingFragmentShaderText = "uniform sampler2D lightSampler;\n"
...
void DynamicLighting::buildObjects()
{
    ...
    for (int ii = 0; ii < (int)mLights.size(); ++ii)
    {
        if (mLights[ii]->getIsEnabled())
        {
            ...
            {
                Warning("Exceeded maximum number of active lights. This light will be "
                        "ignored.");
            }
        }
    }
    ...
    TextureData lightTextureData(2, activeLightCount, 4, TextureData::Texel_F32,
                                 (unsigned char *)&lightData[0]);
