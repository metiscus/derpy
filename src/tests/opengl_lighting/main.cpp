...
int main(void)
{
    ...
    TextureData lightingTexData(2, 3, 4, TextureData::Texel_F32,
                                (const unsigned char *)lightInfo);
    ...
    static const char *lightingVertexShader = "#version 330\n"
    ...
    static const char *lightingFragmentShader = "#version 330\n"
                                                ...
                                                "	float intensity     = clamp(1.0 - (length(position - "
                                                "lightPosInfo.xyz) / lightPosInfo.w), 0, 1);\n"
        //"	intensity = clamp(intensity, 0.0, 1.0 );\n"
                                                "       color += intensity * 0.8 * vec4(lightColorInfo.xyz, 1.0);\n"
                                                "   }\n"
                                                "   color.w = clamp(color.w, 0., 0.94);\n"
                                                "   vec4 realColor = vec4(texture2D(texSampler, texCoordOut).rgb, "
                                                "1.0f);\n"
                                                "   float mixd = color.w; mixd = clamp(mixd, 0., 0.8); \n"
                                                "   fragColor = mixd * color + mixd * realColor;\n"
                                                "   //fragColor = vec4(texture(texSampler, "
                                                "texCoordOut).rgb,1.0f)+vec4(texCoordOut.rg, 1.0, 1.0f);\n"
                                                "}";
    ...
    br.getCamera()->setView(glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 0.0, 1.0),
    ...
    std::shared_ptr<Uniform> lightingUniform(
